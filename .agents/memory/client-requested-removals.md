...
- The inquiry form (`#tara-inquiry-form`, src/inquiryForm.ts) is INTENTIONAL and distinct from the removed Mautic form — a past cleanup deleted it as "unused" and killed contact delivery. Do not remove it when sweeping form code. The site is now static (GitHub Pages): the form posts to the third-party endpoint in `VITE_FORM_ENDPOINT` and otherwise opens a prefilled e-mail to taradealership@gmail.com.
